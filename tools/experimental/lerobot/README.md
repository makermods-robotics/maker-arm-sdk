# Experimental LeRobot teleop bench acceptance runbook
...
1. **Persist the motor-side watchdog (private state)**
   `python tools/experimental/lerobot/persist_can_timeout.py --ids 3,4,5,6,7`
   Expected: all 5 report "written and saved to flash".
2. **Switch to MIT**
   `python tools/experimental/lerobot/switch_protocol.py --ids 3,4,5,6,7 --to mit`
   -> power-cycle the motors -> verify the private protocol is now unresponsive: `maker-arm scan --max-id 8` (expected: no motors found).
...
6. **Round-trip ferry verification**
   Ctrl-C out of teleoperate -> `python tools/experimental/lerobot/switch_protocol.py --ids 3,4,5,6,7 --to private` -> power-cycle -> `maker-arm scan --max-id 8` (expected: 3~7 all online).
...
power cycle, zero it in the private state with `maker-arm zero --motor ID` instead (which the workflow
