...
from monitor import compute_limits, update_yaml_limits  # noqa: E402
...
def test_compute_limits_guards():
    ...
    assert out[0] is None          # 从未收到数据
    assert out[1] is None          # 行程 0.2 < 0.3，视为没测
    assert out[2] == (-0.95, 1.95)


def test_compute_limits_backoff_collapse():
    # 行程刚过防呆线但 backoff 太大导致 lo>=hi → None
...
YAML = (
    ...
    "  - {motor_id: 2, direction: -1, offset: 0.0, lo: -3.0, hi: 3.0, kp: 80, kd: 3.0}  # 行内注释\n"
)


def test_update_yaml_replaces_only_target_motor():
    new = update_yaml_limits(YAML, {2: (-0.95, 1.95)})
    assert "# 顶部注释要保留" in new and "# 行内注释" in new
...
def test_update_yaml_multiple_and_negative_values():
    ...
    assert "kp: 60" in new and "kd: 3.0" in new  # 其它字段不动
