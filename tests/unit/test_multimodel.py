...
def test_motor_params_table():
    ...
    assert p.RS02.p_max == 12.57  # P mapping is the same as RS00 (new firmware)


def test_encode_mit_rs02_golden():
    # τ=8.5 Nm -> RS02: (8.5+17)*65535/34 = 49151 = 0xBFFF, encoded into ID Bit23~8
    cid, data = p.encode_mit(2, 0.0, 0.0, 10.0, 1.0, 8.5, params=p.RS02)
    assert cid == 0x01BFFF02
    assert data == bytes.fromhex("80008000051F3333")  # pos/vel zero point same as RS00
    # v=22 rad/s -> RS02 encodes to 0xBFFF; RS00 encodes differently (54612) -- proves the lookup table is applied
...
def test_parse_feedback_rs02():
    ...
    # decoding with the wrong table is off by a chunk (22*88/66!=22) -- the classic symptom of picking the wrong model
...
def test_mixed_arm_encodes_per_joint():
    ...
    cid, data = be.sent[-1]
    assert (cid >> 8) & 0xFFFF == 0xBFFF                # τ uses the ±17 table
    assert data[2:4] == bytes.fromhex("BFFF")           # v uses the ±44 table
...
def test_config_model_field(tmp_path):
    ...
    assert cfg.joints[0].model == "RS00"    # default -> RS00, backward-compatible with old YAML
...
def test_maker_arm_config_loads():
    ...
    cfg = ArmConfig.from_yaml(DEFAULT_ARM_CONFIG)
